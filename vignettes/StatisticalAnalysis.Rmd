...
sapply(split(cec17.final, cec17.final$Algorithm), 
   ...
   matrix(ncol = 1, 
          dimnames = list(levels(cec17.final$Algorithm),
                          "p-value")) %>%
   AdjustFormatTable(print.code = TRUE)
```
...
wilcoxon.results <- rNPBST::wilcoxñon.test(cbind(EBO, jSO))
...
results.nppairwise <- matrix(c(results.sign, results.wilcoxon,results.wilcoxonRS),
    ...
    AdjustFormatTable(caption = "Non-parametric pairwise comparison between EBO and jSO",
                      label = "tab:np-pairwise", print.code = TRUE)
```
...
AdjustFormatTable(non.parametric,
                  ...
                  label = "tab:np-tests", print.code = TRUE)
```
...
AdjustFormatTable(results.post.global.holland, print.code = TRUE)
```
...
AdjustFormatTable(post.hoc.grouped, print.code = TRUE,
...
ApplyNPTestPostHoc(cec17.final.10,
  ...
  AdjustFormatTable(caption = "Results $n$ vs $n$. Friedman test + Holland adjust.",
                    label = "tab:nvsn-friedman-holland", print.code = TRUE)
```
...
xtable::xtable(glrt.results$statistic$n.vector) %>% print(include.rownames =FALSE)
```
...
xtable::xtable(bmultiple.results$probabilities) %>% print(include.rownames = F)
```
...
bsr.results.dim10 <- cec17.final %>%
   ...
   tidyr::spread("Algorithm", "Result") %>%
   select(-Benchmark) %>%
   rNPBST:::apply.paired.bayesian(rNPBST::bayesianSignedRank.test, rope.min = -10, rope.max = 10)
```
...
FinalScore %>% xtable::xtable() %>% print(include.row_names = FALSE)
```
