...
of the CEC’17 Special Session and Competition on Single Objective Real
...
results.post.global.holland <- cbind(rownames(results.post.global.holland),
                                    results.post.global.holland) %>% as.data.frame()
rownames(results.post.global.holland) <- NULL
colnames(results.post.global.holland) <- c("Algorithm","Friedman","FriedmanAR","Quade") 
...
page.results <- rNPBST::page.test(-convergence.comparison.lshade+convergence.comparison.jso)
```
...
rNPBST::NP.ApproximateConfidenceInterval(x = conf.interval.PPSO[,1],
...
rNPBST::htest2Tex(rNPBST::wilcoxon.test(conf.interval.matrix))
```
...
```{r, out.width = "500px"}
knitr::include_graphics("img/bayesian-multiple-dim10.eps")
```
...
```{r, out.width = "500px"}
knitr::include_graphics("img/bayesian-multiple-dim30.eps")
```
...
```{r, out.width = "500px"}
knitr::include_graphics("img/bayesian-multiple-dim50.eps")
```
```{r, out.width = "500px"}
knitr::include_graphics("img/bayesian-multiple-dim100.eps")
```
