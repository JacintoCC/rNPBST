...
package javanpst.tests.scale.freund_Ansari_BradleyTest;
...
/**
 * The Freund-Ansari-Bradley test.
 * 
 ...
 *  
 * The Freund-Ansari-Bradley test ranks elements with the following weights:
 * 
 *  1,2,3 ... N/2 ... 3,2,1 for N even
 *  1,2,3 ... (N+1/2) ... 3,2,1 for N odd
 *
...
public class Freund_Ansari_BradleyTest extends StatisticalTest{
	...
	public Freund_Ansari_BradleyTest(DataTable newData){
		...
		if(data.getColumns()!=2){
			System.out.println("Freund-Ansari-Bradley test only can be employed with two samples");
			clearData();
			return;
		}
	...
	private void computePValues(){
		...
		if(even){
			...
			numerator=MNx-(m1*(N+1)*(N+1)/(4.0*N));
	...
	public String printReport(){
		...
		report+="\n************\n";
		report+="Freund-Ansari-Bradley test\n";
		report+="**************\n\n";
		
		report+="Freund-Ansari-Bradley Statistic X: "+nf6.format(MNx)+"\n";
		report+="Freund-Ansari-Bradley Statistic Y: "+nf6.format(MNy)+"\n\n";
