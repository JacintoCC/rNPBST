...
import javanpst.data.readers.distributionReaders.KendallQuantileReaderFormat;
import javanpst.data.readers.distributionReaders.KendallExactReaderFormat;

/**
 * An implementation of the Kendall distribution.
 * 
 ...
 * @version 1.0
 */
public class KendallDistribution extends TestDistribution{
	...
	private static class SingletonHolder { 
		...
		public static final KendallDistribution distribution = new KendallDistribution();
	...
	protected void generateTables(){
		...
		double [] tags={0.100,0.050,0.025,0.010,0.005};
		
		table=new Critical1KeyTable(31,tags);
		
		exactTable= new Aproximate1KeyTable(11,25);
	...
	protected void loadFiles(){
		...
		try{
            ...
            SAXParser sp = spf.newSAXParser();  
           	sp.parse(this.getClass().getResourceAsStream("/tables/Kendall/QuantileTable.xml"), new KendallQuantileReaderFormat());	            
        ...
        try{
            SAXParserFactory spf=SAXParserFactory.newInstance(); 
            SAXParser sp = spf.newSAXParser();       
            sp.parse(this.getClass().getResourceAsStream("/tables/Kendall/ExactTable.xml"), new KendallExactReaderFormat());	            
        }catch(ParserConfigurationException e){                  
        	System.err.println("Parser error");             
        }catch(SAXException e2){                                 
        	System.err.println("SAX error: " + e2.getStackTrace());
	 ...
	 * @return p-value computed
	 */
	public double computeApproximatedProbability(int n,double T){
